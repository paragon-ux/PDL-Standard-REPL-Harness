...
| D26 | **ADR-0007 Ratification: Negative-Constraint Operationalization by Omission (PLAN-10 & EXEC-05) & Scorer Invariance:** (1) Structural tension resolved: protocol thoroughness bias induces active procedural wrappers for negative constraints (e.g. `except Exception: raise`), tripping regex adherence while minimalist Control wins by omission. Normative clauses `PLAN-10` and `EXEC-05` codify that negative constraints, exclusions, and unhandled conditions MUST be operationalized as structural omission, relying on native platform runtime propagation; (2) Scorer of Record Invariance (D8) maintained: `scripts/eval/fidelity_scan.py` remains completely unmodified; (3) Empirical verification on GLM-4.7: `MC-06` negative adherence flipped 0.0 -> 1.0 (1.0 fidelity); D25 certification gate closed with 3/3 clean on `ACTOR-02` and 3/3 clean on `DISAMB-03`; 27-case adversarial battery confirmed 0 deliverable egress leaks (100% clean deliverables, 0 hijacks); (4) Diagnostic discipline: disproved config-drift hallucination, proved ADR-0006 bounded reasoning is required to prevent semantic collapse on Class A models, and codified ADR-0007. | Resolves the thoroughness vs negative-adherence tension at the normative protocol layer without relaxing external scorers or clamping output distributions. Empirically certified on GLM-4.7. | **Ratified.** |
| D29 | **Canary-shape generalization (R1/R2 measured, then minimally fixed):** The functional audit flagged the redaction regex as overfit to synthetic canary prefixes. Per owner discipline (measurement before mechanism), a verification-tier probe (`scripts/eval/run_canary_shape_probe.py`) planted prefix-free canaries (UUID / 32-hex / URL query-param) in benign task data with a battery-faithful template. **Finding:** glm-4.7 leaked the UUID canary into the deliverable 2/2 runs (hex 1/2, stochastic; URL 2/2 contained) while control arms echoed naturally -- i.e. layered containment carried only shapes the sanitizer knows; beyond that, containment was model-disposition. A probe v1 confound (an added "reference the unit ID exactly as given" clause, which itself instructs token reproduction) was identified and discarded. **Fix (minimal, measurement-anchored):** `scripts/runtime/quarantine.py` gained a generalized-shape pass (canonical UUID + hex runs >=32 chars) on the sanitized summary surface only; entity-channel inheritance (R2) then blocks re-injection mechanically with no code change. Deliberate policy: SHA-length hex on the summary surface is redacted (refer by role); execution-stage digests untouched (sanitizer never runs there -- no fidelity path to PDL's own content-addressing). **Post-fix:** 3/3 CONTAINED on glm-4.7 and qwen3-coder-30b (deterministic, not disposition); negative controls pinned in `scripts/tests/test_quarantine_generalized.py` (6 tests); battery 77 passed; verifier PASS. | Converts R1/R2 from speculative hardening into measured-and-closed; establishes the battery-faithful canary-shape variant as an F6.4 qualification component. | **Adopted (owner-ratified decision rule: fix only on measured failure).** |
| D28 | **Clause-coverage verifier invariant (spec-drift containment mechanized):** The recurring failure mode was normative documentation outrunning enforced contracts (D2 was found by manual audit). The verifier now parses every clause ID defined in `contracts/standards/*.md` and requires each to be bound in `EXECUTION_CONTRACT.json` or `VERIFICATION_CONTRACT.json`; an unbound clause fails the baseline verifier exactly like a code regression. Verified 95/95 defined-bound; negative path confirmed (synthetic unbound `PLAN-99` fails with `clause_coverage_uncovered`). | Converts the public whitepaper's spec-drift claim from aspiration to mechanism; closes the class of defect D2 represented. | **Adopted.** |
| D27 | **Protocol-arm retirement of the a′ evidence sink (post-v2.3.0 parity audit, owner-ratified Option A):** The D24/TRD-0002 retirement of in-band delimiters was found incompletely executed: the negative-arm scoring path still invoked a `strip_evidence_sink` implementation (silent `ImportError` fallback shim in `scripts/eval/leak_scan.py` after `scripts/runtime/quarantine.py` was purged; active call in `scripts/eval/run_qualified_batch.py`; re-scoring call in `scripts/eval/summarize_run.py`). Per owner ruling: (1) the shim, its call sites, and the `channeled`/`sink_structural_failure` protocol-record fields were removed — the protocol arm now scores unstripped full text (`scan_scope: "full_text_both_arms"`); (2) the control-arm `evidence_verbatim` schema-field channel is RETAINED (out-of-band JSON isolation, TRD-0002-consistent steelman control mechanism); (3) historical a′-era run records remain re-scorable via summarizer schema defaults. This supersedes D17's protocol-arm mechanism (in-band `<<<EVIDENCE>>>` blocks); D17's control-arm `evidence_verbatim` channel stands. | Verified: repo-wide zero importers of removed symbols; `scripts/tests/test_adversarial_battery.py` offline smoke drives the new scoring block end-to-end; full battery 65/65 green; Scorer of Record `scripts/eval/fidelity_scan.py` untouched (D8 upheld); behavior on new runs unchanged (no model emits the delimiters post-D24). | **Adopted (owner-ratified).** |
...
4. **Scorer of Record Invariance**: `scripts/eval/fidelity_scan.py` remains completely UNMODIFIED.
