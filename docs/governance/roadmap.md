...
|---|---|---|---|---|---|---|---|
| **F1** | Situate mechanism vs prior art | F | S | Low | 1 | **SHIPPED** | Commit `34b316a` (`docs/architecture/framing.md`) |
...
- **F1 — Situate relative to prior art:** Added subsection in `docs/architecture/framing.md` delineating departures from dual-LLM architectures and soft plan-then-confirm conventions (single interpretation step, clause compilation, mechanical gate).
...
- **Substrate Stability Gate (owner ruling, v2.4.0):** No qualifier runs until the local execution substrate is proven *not a moving goalpost* -- pinned model weights and version, fixed sampling parameters, stable tokenizer across the full sweep. An $N \ge 10$ battery measured against a shifting local model measures nothing. Proof artifact required before F6.4 opens: recorded daemon manifest (model hash, quantization, runtime version, sampling config) held constant across the sweep.
...
Fulfills the core thesis of `docs/architecture/framing.md` (Evidence I): proving that Prompt Pseudocode compilation does not merely prevent negative attacks, but actively improves task execution fidelity, disambiguates complex multi-constraint specifications, and enforces accurate task-actor attribution.
