...
from scripts.eval.control_api_call import execute_turn, resolve_api_key, TASK_EXECUTION_SYSTEM_PROMPT
from scripts.eval import leak_scan
from scripts.eval import fidelity_scan
...
def main():
    ...
    for c in CASES_ADV:
        ...
        ws_path = RUNS_ROOT / f"probe_{clean_name}" / c["id"]
        if ws_path.exists():
            import shutil
