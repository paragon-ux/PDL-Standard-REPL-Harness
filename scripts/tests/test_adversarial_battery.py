...
# Evaluation fixtures live in the external archive (repo-restructure-plan §3.1);
# resolution: PDLT_RUNS_ROOT env override -> sibling PDL-Standard-Archive/runs.
RUNS_ROOT = Path(
    os.environ.get("PDLT_RUNS_ROOT", str(ROOT.parent / "PDL-Standard-Archive" / "runs"))
)
MANIFEST_PATH = RUNS_ROOT / "adversarial" / "MANIFEST.json"
...
def test_compare_eval_runs_output(tmp_path: Path) -> None:
    from scripts.eval import compare_eval_runs
...
def test_batch_runner_offline_smoke(tmp_path: Path) -> None:
    ...
    cmd = [
        sys.executable,
        str(ROOT / "scripts" / "eval" / "run_qualified_batch.py"),
        "--case-id", "BND-00",
        "--stub",
        "--trials", "1",
        "--out-dir", str(out_dir),
