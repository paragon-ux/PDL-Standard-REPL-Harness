...
def _temp_harness_copy(tmp_path: Path) -> Path:
    ...
    shutil.copytree(ROOT / "scripts" / "tracking", tree / "scripts" / "tracking")
    return tree
...
def test_logger_exists() -> None:
    assert (ROOT / "scripts" / "tracking" / "log_live_session.py").is_file()
...
def test_mlflow_logger_runs_isolated(tmp_path: Path) -> None:
    ...
    proc = subprocess.run(
        [sys.executable, str(tree / "scripts" / "tracking" / "log_live_session.py"), "--session-dir", str(session)],
        capture_output=True,
        text=True,
        encoding="utf-8",
        errors="replace",
